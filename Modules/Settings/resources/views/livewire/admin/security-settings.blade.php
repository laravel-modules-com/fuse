        ...
        <x-form wire:submit="update" method="put">
            ...
            <table>
                ...
                @foreach($ips as $index => $row)
                    ...
                    <tr>
                        <td>
                            <x-form.input wire:model.live="ips.{{ $index }}.ip"
                                          label="none">{{ $row['ip'] }}</x-form.input>
                        </td>
                        <td>
                            <x-form.input wire:model.live="ips.{{ $index }}.comment"
                                          label="none">{{ $row['comment'] }}</x-form.input>
                        </td>
                        <td class="flex justify-center pt-2">
                            <button type="button" wire:click="remove({{ $index }})" class="error">X</button>
                        </td>
                    </tr>
                @endforeach
            </table>
            ...
            <p>
                <x-button color="blue" wire:click="add">{{ __('Add Row') }}</x-button>
